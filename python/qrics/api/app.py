...
class QricsApiApp:
    ...
    def preview_simulation(
        ...
        status = ControlStatusResponse(
            ...
            gait_name=summary.gait_name,
            gait_phase=summary.gait_phase,
            gait_step_frequency_hz=summary.gait_step_frequency_hz,
            swing_foot_count=summary.swing_foot_count,
            stance_foot_count=summary.stance_foot_count,
            joint_command_count=summary.joint_command_count,
            active_target_id=summary.active_target_id,
            reached_target_ids=summary.reached_target_ids,
            target_count=summary.target_count,
            reached_target_count=summary.reached_target_count,
        ...
        self._append_event(
            ...
            payload={
                ...
                "reached_target_ids": list(status.reached_target_ids),
                "target_count": status.target_count,
                "reached_target_count": status.reached_target_count,
                "route_completed": status.route_completed,
                "route_progress_ratio": status.route_progress_ratio,
                "target_distance_m": status.target_distance_m,
                "effective_step_count": status.effective_step_count,
                "requested_step_count": status.requested_step_count,
                "estimated_required_step_count": status.estimated_required_step_count,
                "preview_only": True,
            },
        )
    ...
    def submit_task(
        ...
        if not parsed_task.accepted:
            rejected = TaskPreviewResponse(
                ...
                waypoints=waypoints,
                selected_policy_reason="自然语言解析未生成可执行任务图",
                risk_summary=_task_risk_summary(parsed_task),
                operator_action_required=True,
                scene_ref=payload.scene_ref,
                parser_version=parsed_task.parser_version,
                parse_confidence=parsed_task.confidence,
                constraints=parsed_task.avoid_zone_ids,
                fallback_action=parsed_task.fallback_action,
                explanation=parsed_task.explanations,
                task_script=cast(JsonDict, parsed_task.to_task_script_json()),
                task_graph=cast(JsonDict, parsed_task.to_task_graph_json()),
                rejection_reason=parsed_task.safety_rejection_reason or "任务缺少可执行路径点",
            )
        ...
        preview = TaskPreviewResponse(
            ...
            selected_policy_reason=(
                "自然语言任务解析 -> TaskScript/TaskGraph；"
                "规则策略选择：flat/gravel/platform 占位策略"
            ),
            risk_summary=_task_risk_summary(parsed_task),
            operator_action_required=payload.require_confirmation or parsed_task.needs_confirmation,
            scene_ref=payload.scene_ref,
            parser_version=parsed_task.parser_version,
            parse_confidence=parsed_task.confidence,
            constraints=parsed_task.avoid_zone_ids,
...
def _waypoint_views_from_parsed_task(parsed_task: ParsedTaskDraft) -> tuple[WaypointView, ...]:
    return tuple(
        WaypointView(
        ...
        for waypoint in parsed_task.waypoints
    )


def _task_risk_summary(parsed_task: ParsedTaskDraft) -> str:
    if parsed_task.safety_rejection_reason:
        return f"已拒绝：{parsed_task.safety_rejection_reason}"
    parts = [f"解析置信度 {parsed_task.confidence:.2f}"]
    if parsed_task.avoid_zone_ids:
        parts.append("禁行/避让约束：" + ", ".join(parsed_task.avoid_zone_ids))
    else:
        parts.append("未绑定禁行区；执行前仍需 Safety Shield 门控")
    parts.append(f"回退动作：{parsed_task.fallback_action}")
    return "；".join(parts)
